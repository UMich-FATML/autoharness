...
def main():
    ...
    assert payload["triangle_feature_dim"] >= 7

    train_edges = parse_edge_text(TRAIN_DATA)
    num_nodes = num_nodes_for_edges(train_edges)
    relations, edge_lookup = build_relations(num_nodes, train_edges)

    samples = [
        np.asarray(components.build_triangle_features(source, target, relations, edge_lookup), dtype=float)
        for source, target, _ in train_edges[:12]
    ]
    non_zero = any(np.linalg.norm(sample) > 1e-9 for sample in samples)
    assert non_zero, "triangle features are still all zeros"
