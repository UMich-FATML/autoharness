...
void RDPAccountant::setup_approximation() {
    ...
    const int num_steps = 25.0;
    const int hess_num_steps = params->newton_boosting ? 1000 : 1;
    const double max_gradient_length = std::min(params->l2_threshold, params->pf_l2_threshold);
...
double RDPAccountant::approximate_rho(double individual_sens, double hess_individual_sens) {
    double rho = (tree_params->rho_approx_table.lower_bound(std::pair<const double, const double>(individual_sens, hess_individual_sens)))->second;
    return rho;
}
