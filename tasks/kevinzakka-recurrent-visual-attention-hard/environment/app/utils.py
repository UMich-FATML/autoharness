...
def fit_linear_regression(features: np.ndarray, targets: np.ndarray, regularization: float):
    augmented = np.concatenate([features, np.ones((features.shape[0], 1), dtype=float)], axis=1)
    gram = augmented.T @ augmented + regularization * np.eye(augmented.shape[1], dtype=float)
...
def snap_to_candidates(location: np.ndarray, candidates: np.ndarray):
    distances = np.sum((candidates - location) ** 2, axis=1)
    index = int(np.argmin(distances))
    return np.asarray(candidates[index], dtype=float), index
