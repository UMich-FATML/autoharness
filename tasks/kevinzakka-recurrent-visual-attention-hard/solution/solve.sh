...
def glimpse_network(phi: np.ndarray, location: np.ndarray, scales: int = 1) -> np.ndarray:
    # Oracle implementation of the paper's what/where glimpse encoder.
    if scales == 1:
...
def core_step(previous_state: np.ndarray, glimpse_vector: np.ndarray) -> np.ndarray:
    # Oracle implementation of the recurrent hidden-state update.
    return _relu(previous_state @ PARAMS["w_h"] + glimpse_vector @ PARAMS["w_g"] + PARAMS["b_h"])


def location_network(state: np.ndarray, weights: np.ndarray, bias: np.ndarray) -> np.ndarray:
    # Oracle implementation of the paper-style linear location head.
    return np.clip(state @ weights + bias, -1.0, 1.0)
''',
    encoding="utf-8",
)
PY
