...
class LexMinimizerCountingFunction(object):
    ...
    __slots__ = (
        ...
        'a_max',
        'number_of_greater_letters',
        'number_of_greater_words',
        'postmer_small_values_alphabet_zero',
        'postmer_small_values_alphabet_not_zero',
        'postmer_prefix_letters_vectors',
        'postmer_a_max',
    )
    ...
    def __init__(self, string, alphabet={'A', 'T', 'C', 'G'}, number_of_greater_letters_dic=None):
                ...
                if not flag:
                    ...
                    if cand == ref:
                        ...
                        self.prefix_letters_vectors[i + 1][self.minimizer[i - j + 1]] = min(
                            self.prefix_letters_vectors[i + 1][self.minimizer[i - j + 1]],
                            j + 1,
                        )
        ...
        self.postmer_prefix_letters_vectors: list[dict[str, Callable[[int], int]]] = [
            dict() for _ in range(self.length + 1)
        ]
        ...
        for i in range(1, self.length + 1):
            ...
            for a in self.alphabet:
                if self.prefix_letters_vectors[i][a] == 0:
                    ...
                    self.postmer_prefix_letters_vectors[i][a] = (
                        lambda x, t=self.prefix_letters_vectors[i][a], m=self.length: t * (x >= m + t - 1)
                    )
            ...
            self.postmer_a_max[i] = (
                lambda x, inter=tuple(intervals), bval=tuple(beta_values): ''.join(
                    [inter[j] * (bval[j] <= x < bval[j + 1]) for j in range(len(bval) - 1)]
                )
                + inter[-1] * (x >= bval[-1])
            )
    ...
    def antemer_lower_bound(self, alpha):
        ...
        for j in range(1, alpha + 1):
            ...
            for i in range(1, self.antemer_max_prefix_size):
                if j - (i + 1) >= 0:
                    array[j] += min(
                        self.number_of_greater_letters[self.minimizer[i]],
                        self.number_of_greater_letters[self.a_max[i]],
                    ) * array[j - (i + 1)]
                ...
                if self.a_max[i] > self.minimizer[i]:
                    if j - self.prefix_letters_vectors[i][self.a_max[i]] >= 0:
                        array[j] += array[j - self.prefix_letters_vectors[i][self.a_max[i]] + 1] - (
                            self.number_of_greater_letters[self.minimizer[0]]
                            * array[j - self.prefix_letters_vectors[i][self.a_max[i]]]
                        )
                ...
                if i > j:
                    ...
                    for l in range(i):
                        prod *= (self.autocorrelation_matrix[i - 1][l] == ">") or (
                            self.autocorrelation_matrix[i - 1][l] == "="
                            and self.autocorrelation_matrix[self.length - 1][i - l] == "<"
                        )
                    array_prefix[i][j] = prod
                else:
                    array_prefix[i][j] = min(
                        self.number_of_greater_letters[self.minimizer[i]],
                        self.number_of_greater_letters[self.a_max[i]],
                    ) * array[j - (i + 1)]

                    if self.a_max[i] > self.minimizer[i]:
                        for new_prefix in range(
                            i - self.prefix_letters_vectors[i][self.a_max[i]] + 2,
    ...
    def postmer_lower_bound(self, beta):
        ...
        for j in range(self.length + 1, beta + 1):
            ...
            for i in range(1, self.length + 1):
                ...
                array[j] += min(
                    self.number_of_greater_letters[self.minimizer[i]],
                    self.number_of_greater_letters[a],
                ) * array[j - (i + 1)]
                if i == self.length:
                    array_prefix[j] += min(
                        self.number_of_greater_letters[self.minimizer[i]],
                        self.number_of_greater_letters[a],
                    ) * array[j - (self.length + 1)]
        ...
        for j in range(self.length + 1, beta + 1):
            ...
            for i in range(1, self.length + 1):
                ...
                if a > self.minimizer[i]:
                    array[j] += array[j - self.postmer_prefix_letters_vectors[i][a](j) + 1] - (
                        self.number_of_greater_letters[self.minimizer[0]]
                        * array[j - self.postmer_prefix_letters_vectors[i][a](j)]
                    )

                if i == self.length:
                    array_prefix[j] += min(
                        self.number_of_greater_letters[self.minimizer[i]],
                        self.number_of_greater_letters[self.postmer_a_max[i](j)],
                    ) * array[j - (self.length + 1)]
                    if a > self.minimizer[i]:
                        array_prefix[j] += array[j - self.postmer_prefix_letters_vectors[i][a](j) + 1] - (
                            self.number_of_greater_letters[self.minimizer[0]]
                            * array[j - self.postmer_prefix_letters_vectors[i][a](j)]
                        )
                ...
                if i > j:
                    ...
                    if i == 0:
                        ...
                        for a in self.postmer_small_values_alphabet_not_zero[i]:
                            for new_prefix in range(
                                i - self.prefix_letters_vectors[i][a] + 2,
                    ...
                    if i == self.length:
                        ...
                        array_prefix[i][j] = self.number_of_greater_letters[self.minimizer[i]] * len(
                            self.alphabet
                        ) ** (j - (i + 1))
                else:
                    if i == 0:
                        ...
                        if a > self.minimizer[i]:
                            for new_prefix in range(
                                i - self.postmer_prefix_letters_vectors[i][a](j) + 2,
                                self.length + 1,
                            ):
                                array_prefix[i][j] += array_prefix[new_prefix][
                                    j - self.postmer_prefix_letters_vectors[i][a](j) + 1
                                ]
    ...
    def kmer(self, k, return_all_values=False):
        ...
        for k_int in range(self.length, k + 1):
            ...
            for beta in range(0, beta_max_int + 1):
                total += antemer_array[k_int - self.length - beta] * postmer_array[beta + self.length]
    ...
    def kmer_upper_bound(self, k):
        ...
        return min((beta_max + 1) * 4 ** (k - self.length), total)
