...
def retina_glimpse(image: np.ndarray, location: np.ndarray, patch_size: int = PATCH_SIZE, scales: int = 1) -> np.ndarray:
    """Baseline stub: always returns a center crop instead of a location-conditioned glimpse."""
    center = np.asarray([0.0, 0.0], dtype=float)
    return _crop_and_resize(image, center, patch_size, patch_size).reshape(-1)
